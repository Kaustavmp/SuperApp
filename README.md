...
Most "AI doc analysis" tools summarize what's there. The harder, more valuable problem is reasoning about what's *not* there, and what *stopped being true*. Two concrete failure modes SuperApp targets:
...
The `sample_data/` folder ships with a real example of the second case: two versions of a data-retention policy where customer-data retention drops from 3 years to 1, transaction-log retention rises from 5 to 7, and access-log retention jumps from 90 days to a full year. SuperApp's pipeline is built to catch exactly this kind of drift automatically, with a reasoning trace attached to every finding.

---

## How it works

SuperApp runs uploaded documents through an eight-stage pipeline, orchestrated by `AnalysisOrchestrator`:

```mermaid
flowchart TD
    A["Upload documents"] --> B["Section-aware chunking<br/>(headings → paragraphs → sentences)"]
    B --> C["Schema Induction (LLM)<br/>infers expected topic taxonomy<br/>for this domain"]
    C --> D["Coverage Diffing (LLM)<br/>checks corpus against every<br/>schema item → covered / partial / missing"]
    B --> E["Claim Extraction (LLM)<br/>pulls atomic, self-contained<br/>claims out of every chunk"]
    E --> F["Vector Store Pruning<br/>ChromaDB + embeddings find<br/>semantically similar claim pairs"]
    F --> G["Contradiction Detection (LLM)<br/>classifies each pair:<br/>supports / contradicts / silent_on / unrelated"]
    G --> H["Claim Graph (NetworkX)<br/>directed graph + contradiction<br/>clustering via connected components"]
    D --> I["Scoring & Ranking<br/>severity × confidence<br/>→ unified Findings"]
    H --> I
...
- **Semantic candidate pruning** — instead of comparing every claim against every other claim ($\mathcal{O}(n^2)$ LLM calls), a ChromaDB vector store pre-filters to only semantically similar pairs before the expensive contradiction check.
...
By default, `LLM_PROVIDER=ollama` — no API key required, but you need a local [Ollama](https://ollama.com/) instance running with `llama3.1` and `nomic-embed-text` pulled:
...
To use Claude or GPT instead, set `LLM_PROVIDER=anthropic` or `openai` and add the matching API key.
...
Visit [http://localhost:8000/](http://localhost:8000/), upload one or more documents (try `sample_data/policy_contradictions/`), and run an analysis.
...
SuperApp is an MVP with its production seams already cut, not yet welded:
